...
const AddDoctor = () => {
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-4xl bg-white p-3 sm:p-8 rounded-2xl border border-slate-100 shadow-sm text-left mx-auto overflow-hidden">
        {/* Top Header Section */}
        <div className="flex flex-row items-center justify-between gap-3 mb-6 border-b border-slate-50 pb-4">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 text-xs sm:text-sm font-semibold text-slate-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl transition-all active:scale-95 cursor-pointer shrink-0"
          >
            <LuArrowLeft className="text-sm" /> Go Back
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-sm font-medium text-slate-600 mb-1 flex items-center gap-1.5">
                <LuActivity className="text-slate-400 text-base" /> Doctor Type / Role
              </label>
              ...
              <div className="relative w-full">
                ...
                >
                  <span className="truncate">
                    {formData.isSurgeon
                      ? "Surgeon (OPD + Operation) 🩺"
                      : "General Physician (OPD) 👨‍⚕️"}
                  </span>
                  <span
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium text-slate-600 mb-1 flex items-center gap-1.5">
                <LuStethoscope className="text-slate-400 text-base" /> Speciality
              </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-sm font-medium text-slate-600 mb-1 flex items-center gap-1.5">
                <LuCalendar className="text-slate-400 text-base" /> Joining Date / Booking Open Date
              </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="text-sm font-medium text-slate-600 mb-1 flex items-center gap-1.5">
                <LuDollarSign className="text-slate-400 text-base" /> OPD Fees (Rs.)
              </label>
              <input
                type="number"
                name="fees"
            ...
            <div>
              <label className="text-sm font-medium text-slate-600 mb-1 flex items-center gap-1.5">
                <LuDollarSign className="text-slate-400 text-base" /> Daily Round Fees (Rs.)
              </label>
              <input
                type="number"
                name="roundFee"
            ...
            <div>
              <label className="text-sm font-medium text-slate-600 mb-1 flex items-center gap-1.5">
                <LuPercent className="text-slate-400 text-base" /> Admin Commission (%)
              </label>
              <input
                type="number"
                name="commission"
          ...
          <div className="border-t border-slate-100 pt-6">
            ...
            <div className="space-y-3 bg-slate-50 p-1.5 sm:p-4 rounded-2xl border border-slate-100">
              {Object.keys(weeklySchedule).map((day) => (
                <div
                  key={day}
                  className="flex flex-row items-center justify-between gap-1 p-2 sm:p-3 bg-white rounded-xl border border-slate-100 shadow-sm hover:border-slate-200 transition-colors overflow-hidden"
                >
                  {/* Day Checkbox & Name */}
                  <div className="flex items-center gap-1 select-none shrink-0 min-w-19 sm:min-w-25">
                    <input
                      className="h-3.5 w-3.5 text-teal-600 focus:ring-teal-500 border-slate-300 rounded cursor-pointer accent-teal-600"
                      type="checkbox"
                      id={`check-${day}`}
                    ...
                    <label
                      htmlFor={`check-${day}`}
                      className="text-[11px] sm:text-sm font-bold text-slate-700 capitalize cursor-pointer truncate"
                    >
                      {day}
                    </label>
                  </div>

                  {/* Duty timing inputs OR Off Day block */}
                  <div className="flex items-center justify-end shrink-0">
                    {weeklySchedule[day].isAvailable ? (
                      <div className="flex items-center gap-1 bg-slate-50/80 p-0.5 sm:p-1 rounded-lg border border-slate-100">
                        <input
                          type="time"
                          ...
                          className="p-1 px-1 rounded bg-white border border-slate-200 text-[10px] sm:text-xs font-semibold text-slate-700 outline-none focus:border-teal-500 cursor-pointer w-17 sm:w-22 text-center"
                        />
                        <span className="text-[9px] sm:text-xs text-slate-400 font-bold px-0.5">
                          to
                        </span>
                        <input
                          type="time"
                      ...
                      <span className="inline-flex items-center gap-0.5 text-[9px] sm:text-xs text-rose-500 font-bold bg-rose-50/60 px-1.5 py-1 rounded-lg border border-rose-100 shrink-0">
                        <LuBan className="text-[9px]" /> Off Day
                      </span>
                    )}
                  </div>
                </div>
